; ====================================================================
; 8051 貪食蛇專案 - 最終完美修正版 (解決 Relative Jump Too Large 問題)
...
; --------------------------------------------------------------------
UPDATE_SNAKE:
        ; A. 依據方向計算出下一格蛇頭位置 (使用反轉邏輯與 LJMP 解決長度限制)
        MOV     A, SNAKE_DIR
        CJNE    A, #0, U_CHK_DN
        
        ; --- 向上移動 ---
        MOV     A, SNAKE_Y
        JNZ     Y_UP_OK             ; 邏輯反轉：如果不是 0 就不會撞牆，跳過死線
        LJMP    GAME_OVER_TRIGGER   ; 否則就用 LJMP 送牠去 Game Over
Y_UP_OK:
        DEC     A
        MOV     NEXT_Y, A
        MOV     NEXT_X, SNAKE_X
...
U_CHK_DN:
        ...
        INC     A
        CJNE    A, #2, Y_DN_OK
        LJMP    GAME_OVER_TRIGGER   ; 超過第 1 列 -> 改用 LJMP
Y_DN_OK:
        MOV     NEXT_Y, A
        MOV     NEXT_X, SNAKE_X
        SJMP    U_CHK_SELF
...
U_CHK_LF:
        ...
        ; --- 向左移動 ---
        MOV     A, SNAKE_X
        JNZ     X_LF_OK             ; 邏輯反轉：如果不是 0 就不會撞牆，跳過死線
        LJMP    GAME_OVER_TRIGGER   ; 否則就用 LJMP
X_LF_OK:
        DEC     A
        MOV     NEXT_X, A
        MOV     NEXT_Y, SNAKE_Y
...
U_CHK_RT:
        ...
        MOV     A, SNAKE_X
        INC     A
        CJNE    A, #16, X_RT_OK     ; 16 = 10H (超出15行) 
        LJMP    GAME_OVER_TRIGGER   ; 撞右牆 -> 改用 LJMP
X_RT_OK:
        MOV     NEXT_X, A
        MOV     NEXT_Y, SNAKE_Y
...
U_CHK_SELF:
        MOV     A, SNAKE_LEN
        DEC     A
        MOV     R0, A               ; R0 = 身體最後一格的索引
        MOV     R2, #1              ; 從第 1 格身體開始檢查
SELF_LOOP:
        MOV     A, R0
        CLR     C
        SUBB    A, R2
        JC      U_CHK_FOOD          ; 如果 R2 > R0，代表全身檢查完都沒事

        ; 檢查預計新頭 (NEXT) 是否與該節身體重合
        MOV     A, #SNAKE_X
        ADD     A, R2
        MOV     R1, A
        MOV     A, @R1
...
; --------------------------------------------------------------------
; ?? GAME OVER 死亡處理觸發器 (關中斷 + 蜂鳴器慘叫 + 顯示分數)
; --------------------------------------------------------------------
GAME_OVER_TRIGGER:
        CLR     TR0                 ; 停止 Timer 0，讓蛇瞬間定格
        CLR     EA                  ; 關閉所有中斷
        
        ACALL   BUZZER_DEATH        ; 呼叫蜂鳴器慘叫子程式
        
        ACALL   LCD_INIT            ; 刷新/清空 LCD
        ...
        DIV     AB                  ; A = 十位數, B = 個位數
        JZ      SHOW_DIGIT_1
        ADD     A, #30H             ; 十位數轉 ASCII
        ACALL   LCD_DATA
SHOW_DIGIT_1:
        MOV     A, B
        ADD     A, #30H             ; 個位數轉 ASCII
        ACALL   LCD_DATA

FREEZE_HERE:
        SJMP    FREEZE_HERE         ; 死結，等待玩家按下 Reset 鍵重新開始

; --------------------------------------------------------------------
; 子程式：死亡蜂鳴器控制 (高頻變低頻慘叫聲)
; --------------------------------------------------------------------
BUZZER_DEATH:
        MOV     R5, #100            ; 第一段：急促高音嗶嗶聲
B_LOOP1:
        CLR     BUZZER              ; 叫
        ACALL   DELAY_1MS
        SETB    BUZZER              ; 停
